...
public abstract class ZRecycleEasyAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> implements View.OnClickListener, View.OnLongClickListener {
    ...
    public boolean onLongClick(View v) {
        ...
        if (selectMode == SelectMode.CLICK) {//长按模式
            if (onItemLongClickListener != null) {
                onItemLongClickListener.onClickedLong(itemPosition);
            }
        }
        return false;
    }
     ...
     * @param itemPosition
     */
    public void clearItemSelected(int itemPosition) {
        if (maxSelectedCount <= 0  //选择不受限制
                || multiSelected.size() < maxSelectedCount) {  // 选择个数需要小于最大可选数
            if (multiSelected.contains(itemPosition)) {
                multiSelected.remove((Integer) itemPosition);
    ...
    public interface OnItemLongClickListener {
         ...
         * @param itemPosition 点击的item位置
         */
        void onClickedLong(int itemPosition);
    }
